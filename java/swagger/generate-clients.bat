...
java -jar swagger-codegen-cli-2.2.2.jar generate -i %AUTH_SERVER_LOCATION%OAuth2Authentication -l java -o client/auth -c auth-config.json
...
java -jar swagger-codegen-cli-2.2.2.jar generate -i %APP_SERVER_LOCATION%ClientRestController -l java -o client/app -c app-config.json
java "-Dmodels" "-Dapis" "-DmodelTests=false" "-DapiTests=false" "-DmodelDocs=false" "-DapiDocs=false" -jar swagger-codegen-cli-2.2.2.jar generate -i %APP_SERVER_LOCATION%InternalRestController -l java -o client/app -c app-config.json